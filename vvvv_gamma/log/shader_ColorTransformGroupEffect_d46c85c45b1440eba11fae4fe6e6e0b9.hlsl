...
@P   - ColorTransformGroup.Transforms: Stride.Rendering.Images.FilmGrain, Stride.Rendering.Images.Vignetting
***************************
****  ConstantBuffers  ****
***************************
...
//
#line 246 "H:\JHH_SpaceMusic\SpaceMusicZH\vvvv-binaries\vvvv_gamma\log\shader_ColorTransformGroupEffect_d46c85c45b1440eba11fae4fe6e6e0b9.hlsl"
...
@G    Pixel => e9da57ac3f89f6c45637b06a2783dd19
//
// Generated by Microsoft (R) HLSL Shader Compiler 10.1
//
//
...
//
#line 184 "H:\JHH_SpaceMusic\SpaceMusicZH\vvvv-binaries\vvvv_gamma\log\shader_ColorTransformGroupEffect_d46c85c45b1440eba11fae4fe6e6e0b9.hlsl"
...
#line 145
mad r0.zw, r0.zzzw, l(0.000000, 0.000000, 2.000000, 2.000000), l(0.000000, 0.000000, -1.000000, -1.000000)  // r0.z <- <rnm_id8 return value>.x; r0.w <- <rnm_id8 return value>.y; r0.z <- <rnm_id8 return value>.x; r0.w <- <rnm_id8 return value>.y; r0.z <- <rnm_id8 return value>.x; r0.w <- <rnm_id8 return value>.y
...
#line 145
mad r0.zw, r0.zzzw, l(0.000000, 0.000000, 2.000000, 2.000000), l(0.000000, 0.000000, -1.000000, -1.000000)  // r0.z <- <rnm_id8 return value>.x; r0.w <- <rnm_id8 return value>.y
...
#line 145
mad r1.zw, r1.zzzw, l(0.000000, 0.000000, 2.000000, 2.000000), l(0.000000, 0.000000, -1.000000, -1.000000)  // r1.z <- <rnm_id8 return value>.x; r1.w <- <rnm_id8 return value>.y; r1.z <- <rnm_id8 return value>.x; r1.w <- <rnm_id8 return value>.y
...
mul r0.xy, r0.xyxx, r1.zwzz  // r0.y <- <Quintic_id13 return value>.x
...
max r0.y, r0.y, l(0.000100)  // r0.y <- <Luma_id12 return value>
...
float2 Quintic_id13(float2 x)
{
    return x * x * x * (x * (x * 6.0 - 15.0) + 10.0);
}
float fade_id9(float t)
{
    return Quintic_id13(t);
}
float4 rnm_id8(float2 tc)
{
...
float pnoise3D_id7(float3 p)
{
    float permTexUnit = 1.0 / 256.0;
    float permTexUnitHalf = permTexUnit * 0.5;
    float3 pi = permTexUnit * floor(p) + permTexUnitHalf;
    float3 pf = frac(p);
    float perm00 = rnm_id8(pi.xy).a;
    float3 grad000 = rnm_id8(float2(perm00, pi.z)).rgb * 4.0 - 1.0;
    float n000 = dot(grad000, pf);
    float3 grad001 = rnm_id8(float2(perm00, pi.z + permTexUnit)).rgb * 4.0 - 1.0;
    float n001 = dot(grad001, pf - float3(0.0, 0.0, 1.0));
    float perm01 = rnm_id8(pi.xy + float2(0.0, permTexUnit)).a;
    float3 grad010 = rnm_id8(float2(perm01, pi.z)).rgb * 4.0 - 1.0;
    float n010 = dot(grad010, pf - float3(0.0, 1.0, 0.0));
    float3 grad011 = rnm_id8(float2(perm01, pi.z + permTexUnit)).rgb * 4.0 - 1.0;
    float n011 = dot(grad011, pf - float3(0.0, 1.0, 1.0));
    float perm10 = rnm_id8(pi.xy + float2(permTexUnit, 0.0)).a;
    float3 grad100 = rnm_id8(float2(perm10, pi.z)).rgb * 4.0 - 1.0;
    float n100 = dot(grad100, pf - float3(1.0, 0.0, 0.0));
    float3 grad101 = rnm_id8(float2(perm10, pi.z + permTexUnit)).rgb * 4.0 - 1.0;
    float n101 = dot(grad101, pf - float3(1.0, 0.0, 1.0));
    float perm11 = rnm_id8(pi.xy + float2(permTexUnit, permTexUnit)).a;
    float3 grad110 = rnm_id8(float2(perm11, pi.z)).rgb * 4.0 - 1.0;
    float n110 = dot(grad110, pf - float3(1.0, 1.0, 0.0));
    float3 grad111 = rnm_id8(float2(perm11, pi.z + permTexUnit)).rgb * 4.0 - 1.0;
    float n111 = dot(grad111, pf - float3(1.0, 1.0, 1.0));
    float4 n_x = lerp(float4(n000, n001, n010, n011), float4(n100, n101, n110, n111), fade_id9(pf.x));
    float2 n_xy = lerp(n_x.xy, n_x.zw, fade_id9(pf.y));
    float n_xyz = lerp(n_xy.x, n_xy.y, fade_id9(pf.z));
    return n_xyz;
}
float2 coordRot_id6(float2 tc, float angle)
{
...
float4 Compute_id4(inout PS_STREAMS streams, float4 color)
{
    ...
    float2 rotCoordsR = coordRot_id6(texCoord, Time_id76);
    float2 newCoord = rotCoordsR / Texture0TexelSize_id15 / GrainSize_id77;
    float n = pnoise3D_id7(float3(newCoord, 0.0));
    float3 noiseFactor = float3(n, n, n);
    float3 col = color.rgb;
    float luminance = lerp(0.0, Luma_id12(col), LuminanceFactor_id78);
