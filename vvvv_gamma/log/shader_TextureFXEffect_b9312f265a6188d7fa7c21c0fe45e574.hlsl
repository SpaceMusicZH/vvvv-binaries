...
@P   - EffectNodeBase.EffectNodeBaseShader: mixin BlurPass2_Internal_TextureFX [{Control = mixin DoFloat4 [{Before = mixin ComputeOrder [{Computes = [mixin AssignVarFloat4<Default_1> [{Value = InputFloat4<ShaderFX.InputValueFloat4,PerUpdate>}]]}]}, {Value = GetVarFloat4<Default_1>}]}, {Strength = mixin DoFloat [{Before = mixin ComputeOrder [{Computes = [mixin AssignVarFloat<Input_0> [{Value = InputFloat<ShaderFX.InputValueFloat,PerUpdate>}]]}]}, {Value = GetVarFloat<Input_0>}]}]
@P Strength: mixin DoFloat [{Before = mixin ComputeOrder [{Computes = [mixin AssignVarFloat<Input_0> [{Value = InputFloat<ShaderFX.InputValueFloat,PerUpdate>}]]}]}, {Value = GetVarFloat<Input_0>}]
@P Control: mixin DoFloat4 [{Before = mixin ComputeOrder [{Computes = [mixin AssignVarFloat4<Default_1> [{Value = InputFloat4<ShaderFX.InputValueFloat4,PerUpdate>}]]}]}, {Value = GetVarFloat4<Default_1>}]
***************************
****  ConstantBuffers  ****
***************************
...
*****     Sources     *****
***************************
@S    BlurPass2_Internal_TextureFX => 26b8a9eb5ad161fc028cd7543f2b709c
...
//
#line 241 "H:\JHH_SpaceMusic\SpaceMusicZH\vvvv-binaries\vvvv_gamma\log\shader_TextureFXEffect_b9312f265a6188d7fa7c21c0fe45e574.hlsl"
...
@G    Pixel => aeafbd0e7ab3d692174f686483c24908
//
// Generated by Microsoft (R) HLSL Shader Compiler 10.1
//
//
...
//
#line 204 "H:\JHH_SpaceMusic\SpaceMusicZH\vvvv-binaries\vvvv_gamma\log\shader_TextureFXEffect_b9312f265a6188d7fa7c21c0fe45e574.hlsl"
div r0.xyzw, l(0.500000, 0.500000, 0.500000, 0.500000), cb1[1].xyxy

#line 203
max r1.x, cb1[1].y, cb1[1].x
log r1.x, r1.x
mul r1.x, r1.x, cb0[1].x  // r1.x <- lod
...
#line 204
mul r3.xyzw, r0.xyzw, r1.yyyy  // r3.z <- off.x; r3.w <- off.y
...
{
    streams.Input_0_id95 = Compute_id43();
}
float4 Compute_id34()
{
    return InputValue_id90;
}
float Compute_id49(inout PS_STREAMS streams)
{
    return streams.Input_0_id95;
}
...
{
    streams.Default_1_id89 = Compute_id34();
}
float Compute_id33(inout PS_STREAMS streams)
{
    Compute_id48(streams);
    return Compute_id49(streams);
}
float4 Compute_id40(inout PS_STREAMS streams)
{
    return streams.Default_1_id89;
}
...
void ClacLodAndOffset_id29(inout PS_STREAMS streams, float2 R, float strenght, out float lod, out float2 off)
{
    lod = Compute_id33(streams) * log2(max(R.x, R.y));
    off = 0.5 / R * pow(2, lod) * saturate(lod);
}
float4 Compute_id32(inout PS_STREAMS streams)
{
    Compute_id39(streams);
    return Compute_id40(streams);
}
float4 Filter_id31(inout PS_STREAMS streams, float4 tex0col)
{
    float lod;
    float2 off;
    ClacLodAndOffset_id29(streams, ViewSize_id77, Compute_id33(streams), lod, off);
...
float4 Shading_id28(inout PS_STREAMS streams)
{
    ...
    return lerp(col, Filter_id31(streams, col), Compute_id32(streams));
}
