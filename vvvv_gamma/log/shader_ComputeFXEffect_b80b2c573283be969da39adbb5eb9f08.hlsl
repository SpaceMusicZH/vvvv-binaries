...
//
#line 188 "H:\JHH_SpaceMusic\SpaceMusicZH\vvvv-binaries\vvvv_gamma\log\shader_ComputeFXEffect_b80b2c573283be969da39adbb5eb9f08.hlsl"
...
#line 97
    ...
    mad r8.xy, r7.zyzz, r4.xzxx, r5.zxzz  // r8.x <- <rot3D_id53 return value>._m02; r8.y <- <rot3D_id53 return value>._m10
    mad r9.xy, r7.xwxx, r4.yzyy, r6.xxxx  // r9.x <- <rot3D_id53 return value>._m00; r9.y <- <rot3D_id53 return value>._m11
    mad r8.z, r7.w, r4.x, -r5.y  // r8.z <- <rot3D_id53 return value>._m12
    mad r10.xy, r7.yzyy, r4.zxzz, -r5.xzxx  // r10.x <- <rot3D_id53 return value>._m01; r10.y <- <rot3D_id53 return value>._m20
    mad r10.z, r7.w, r4.x, r5.y  // r10.z <- <rot3D_id53 return value>._m21
    mul r4.x, r4.x, r4.x
    mad r8.w, r4.x, r4.w, r6.x  // r8.w <- <rot3D_id53 return value>._m22
...
void FlatMeshLine_id54(RWStructuredBuffer<Pos3Norm3Col4Tex2Tan4Id2> outputBuffer, uint id, float3 pos, float3 tan, float scale, float4 color, float3 posNext, float3 tanNext, float scaleNext, float4 colorNext, float radius, int sampleCnt, bool normalBend, float3 up = float3(0, 1, 0), float3 upNext = float3(0, 1, 0))
{
...
void TubeMeshLine_id56(RWStructuredBuffer<Pos3Norm3Col4Tex2Tan4Id2> outputBuffer, uint id, float3 pos, float3 tan, float scale, float4 color, float3 posNext, float3 tanNext, float scaleNext, float4 colorNext, float radius, int sampleCnt, int resolution, float3 up = float3(0, 1, 0))
{
    Pos3Norm3Col4Tex2Tan4Id2 v0, v1, v2, v3;
    ...
    float3x3 rNext = rot3D_id53(normalize(tanNext), TWOPI_id37 / resolution);
    v0.Position = pos + dir * radius * .5 * scale;
    v1.Position = posNext + dirNext * radius * .5 * scaleNext;
    v0.Color = v2.Color = color;
    v1.Color = v3.Color = colorNext;
...
LineAttribute Compute_id59(float3 p, float id)
{
...
void Compute_id58(inout CS_STREAMS streams)
{
    ...
    uint n = min(sbSize3_id4(FromBuffer_id43), sbSize3_id4(ToBuffer_id44));
    if (dtid >= n)
        return;
    ...
    LineAttribute fromAttribute = Compute_id59(from, dtid);
    LineAttribute toAttribute = Compute_id59(to, dtid);
    ...
    if (MeshType_id49 == 0)
    {
        FlatMeshLine_id54(outputBuffer_id45, dtid, from, t, fromScale, fromAttribute.c, to, t, toScale, toAttribute.c, Radius_id46, 2, normalize(fromViewDir), normalize(toViewDir));
    }
    else
    {
        TubeMeshLine_id56(outputBuffer_id45, dtid, from, t, fromScale, fromAttribute.c, to, t, toScale, toAttribute.c, Radius_id46, 2, Resolution_id47);
    }
}
[numthreads(1, 1, 1)]
